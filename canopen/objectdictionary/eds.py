    ...
    for section in eds.sections():
        ...
        if match is not None:
            ...
            if object_type in (VAR, DOMAIN):
                ...
                last_subindex = objectdictionary.Variable("Number of entries", index, 0)
        ...
        if match is not None:
            ...
            entry = od[index]
            if isinstance(entry, (objectdictionary.Record, objectdictionary.Array)):
...
def _convert_variable(node_id, var_type, value):
    if var_type in objectdictionary.datatypes.DATA_TYPES:
        return value
    elif var_type in objectdictionary.datatypes.FLOAT_TYPES:
        return float(value)
    else:
        # COB-ID can have a suffix of '$NODEID+' so replace this with node_id
        # before converting
...
def build_variable(eds, section, node_id, index, subindex=0):
    ...
    if var.data_type > 0x1B:
        # The object dictionary editor from CANFestival creates an optional
        # object if min max values are used.
        # This optional object is then placed in the eds under the section [A0]
        # (start point, iterates for more).
        # The eds.get function gives us 0x00A0 now convert to String without
        # hex representation and upper case.
        ...
        try:
            ...
            logger.warning(
                "%s has an unknown or unsupported data type (%X)", name, var.data_type
            )
            # Assume DOMAIN to force application to interpret the byte data
            var.data_type = objectdictionary.datatypes.DOMAIN
