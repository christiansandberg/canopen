...
class NmtBase(object):
    ...
    def send_command(self, code):
        ...
        if code in COMMAND_TO_STATE:
            ...
            logger.info(
                "Changing NMT state from %s to %s",
                NMT_STATES[self._state],
                NMT_STATES[new_state],
            )
            self._state = new_state
...
class NmtMaster(NmtBase):
    ...
    def start_node_guarding(self, period):
        ...
        :param float period:
            Period (in seconds) at which the node guarding should be advertised
            to the slave node.
        """
...
class NmtSlave(NmtBase):
    ...
    def start_heartbeat(self, heartbeat_time_ms):
        ...
        if heartbeat_time_ms > 0:
            logger.info(
                "Start the hearbeat timer, interval is %d ms", self._heartbeat_time_ms
            )
